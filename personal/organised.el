...
(defun better-org-open-and-search ()
  "Open file at point and search for string that follws the ::"
  (interactive)
  ;; (message (format " open and search - beginning"))
  (let ((line (thing-at-point 'filename)))
    ;; (message (format " open and search - got line %s" line))
    (let ((line-components (split-string line "::")))
      ;; (message (format " open and search - got line components  %s" line-components))
      (let ((search (nth 1 line-components)))
        ;; (message (format " open and search - got search  %s" search))
        (let ((file-components (split-string (nth 0 line-components) ":")))
          ;; (message (format " open and search - got file components %s" file-components))
          (let ((file (nth 0 file-components))
                (path (nth 1 file-components)))
            ;; (message (format " open and search - got line components zzz %s > %s > %s" file path search))

            (progn (org-link-open-as-file path t)
                   (beginning-of-buffer)
